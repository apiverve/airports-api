...
class AirportsValidationException implements Exception {
  final List<String> errors;

  AirportsValidationException(this.errors);

  @override
  String toString() => 'AirportsValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/airports?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [iata] (required) - The IATA code of the airport for which you want to get the information (e.g., MCI)
class AirportsClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'iata': ValidationRule(type: 'string', required: true),
  };

  AirportsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/airports',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AirportsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AirportsValidationException(errors);
    }
  }

  /// Execute a request to the Airports Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<AirportsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AirportsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AirportsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AirportsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AirportsException('Resource not found');
      } else {
        throw AirportsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AirportsException) rethrow;
      throw AirportsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Airports Lookup API client.
class AirportsException implements Exception {
  final String message;

  AirportsException(this.message);

  @override
  String toString() => 'AirportsException: $message';
}
